...
export class ReActAgent implements Agent {
  ...
  async processMessage(
    ...
    try {
      ...
      return {
        content:
          "I'm sorry, I encountered an error while processing your request. Could you please try again or rephrase your request?",
        tokenCount: null,
        toolResults: [],
      };
    }
  ...
  private async handleComplexMessage(message: string): Promise<Response> {
    ...
    try {
      const lastStep = await this.engine.getLastReasoningStep(
        this.state.config.userId,
      );
      if (lastStep) {
  ...
  private isSimpleGreeting(message: string): boolean {
    ...
    return (
      simpleGreetings.includes(normalizedMessage) ||
