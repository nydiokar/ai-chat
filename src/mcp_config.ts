...
const features: MCPConfig["features"] = {
  core: {
...
const createMCPConfig = (): MCPConfig => {
  ...
  const enabledServers = Object.entries(servers).reduce(
    (acc, [key, config]) => {
      // Check if server is explicitly disabled
      const isDisabled =
        process.env[`MCP_${key.toUpperCase().replace(/-/g, "_")}_DISABLED`] ===
        "true";
      ...
      if (!isDisabled) {
        ...
        Object.entries(config.env || {}).forEach(([envKey, envValue]) => {
          if (
            !envValue &&
            (envKey.endsWith("_TOKEN") || envKey.endsWith("_KEY"))
          ) {
            warn(
              `Server '${key}' missing ${envKey} - some features may be unavailable`,
            );
          }
        });
      }
...
export const mcpConfig: MCPConfig = createMCPConfig();
