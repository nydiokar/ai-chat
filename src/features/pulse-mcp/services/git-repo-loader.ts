...
export class GitRepoLoader {
  ...
  static async cloneRepository(repoUrl: string): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `Repository ${repoName} already exists at ${repoPath}, updating instead`,
        );
  ...
  static async updateRepository(repoPath: string): Promise<string> {
    try {
      ...
      console.error(
        `Failed to update repository at ${repoPath}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  static async buildRepository(repoPath: string): Promise<boolean> {
    try {
      console.log(
        `Installing dependencies and building repository at ${repoPath}`,
      );
      ...
      console.error(
        `Failed to build repository at ${repoPath}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  static async findEntryPoint(repoPath: string): Promise<string> {
    try {
      ...
      throw new Error(
        `Could not find entry point for repository at ${repoPath}`,
      );
    } catch (error: any) {
  ...
  static createServerConfig(
    ...
    const serverId = pulseServer.name
      .toLowerCase()
      .replace(/[^a-z0-9-]/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "");
   ...
   */
  static async prepareFromGitHub(
    pulseServer: PulseMCPServer,
    ...
    try {
      ...
      const repoPath = await GitRepoLoader.cloneRepository(
        pulseServer.source_code_url,
      );
      ...
      return GitRepoLoader.createServerConfig(
        pulseServer,
        repoPath,
        entryPoint,
      );
