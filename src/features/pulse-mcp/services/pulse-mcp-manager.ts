...
export class PulseMCPManager {
  ...
  async addServerToConfig(
    ...
    if (config.mcpServers && config.mcpServers[serverId]) {
      console.log(
        `Server with ID ${serverId} already exists in configuration. Skipping.`,
      );
    ...
    if (!pulseServer.source_code_url) {
      console.error(
        `Server ${serverId} does not have a source code URL. Cannot install.`,
      );
      return config; // Return unchanged config
    }

    console.log(
      `Adding server ${serverId} (${pulseServer.name}) to configuration from GitHub`,
    );
    ...
    try {
      ...
      if (!serverConfig) {
        console.error(
          `Could not create config for server ${serverId}. Skipping.`,
        );
        return config;
      }
  ...
  async findAndAddServers(
    ...
    for (const server of servers) {
      ...
      if (
        Object.keys(updatedConfig.mcpServers).length >
        Object.keys(currentConfig.mcpServers).length
      ) {
