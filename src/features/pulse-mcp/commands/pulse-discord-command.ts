...
export const pulseCommand = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand.setName("list").setDescription("List all available MCP servers"),
  );
...
async function saveConfig(updatedConfig: MCPConfig): Promise<void> {
  try {
    ...
    const newServers = Object.entries(updatedConfig.mcpServers).filter(
      ([id]) => id !== "github" && id !== "brave-search",
    );
    ...
    console.log(
      `Adding ${newServers.length} servers to config: ${newServers.map(([id]) => id).join(", ")}`,
    );

    // First check if we already have a section for dynamically added servers
    const dynamicSectionStartMarker =
    ...
    if (currentContent.includes(dynamicSectionStartMarker)) {
      ...
      if (startIndex !== -1 && endIndex !== -1) {
        const beforeSection = currentContent.substring(
          0,
          startIndex + dynamicSectionStartMarker.length,
        );
        ...
        console.log(
          "Could not find proper section boundaries, appended to end",
        );
      }
    } else {
    ...
    try {
      ...
      const refreshedConfig = await import(
        "../../../mcp_config.js?t=" + Date.now()
      );
      ...
      console.warn(
        "Changes were saved to file but may not be reflected until restart",
      );
    }
...
async function getServerManager() {
  ...
  const { DiscordService } = await import(
  ...
  try {
    ...
    if (serverManager.hasServer(serverId)) {
      ...
      if (server?.state === ServerState.RUNNING) {
        ...
        isEnhancedServerManager(serverManager)
      ) {
...
function createServerEmbed(
  ...
  // Create embed
  const embed = new EmbedBuilder().setTitle(server.name).setColor("#0099ff");
 ...
 */
export async function handlePulseCommand(
  ...
  if (subcommand === "search") {
    ...
    try {
      ...
      const collector = interaction.channel?.createMessageComponentCollector({
        filter: (i) =>
          i.customId.startsWith("install-server-") &&
      ...
      collector?.on("collect", async (buttonInteraction) => {
        ...
        const serverIndex = parseInt(
          buttonInteraction.customId.replace("install-server-", ""),
        );
        ...
        const serverId = serverToInstall.name
          .toLowerCase()
          .replace(/[^a-z0-9-]/g, "-")
          .replace(/-+/g, "-")
          .replace(/^-|-$/g, "");
        ...
        try {
          ...
          const startCollector =
            interaction.channel?.createMessageComponentCollector({
              filter: (i) =>
                i.customId === `start-server-${serverId}` &&
                i.user.id === interaction.user.id,
              time: 60000, // 1 minute
            });
      ...
      await interaction.followUp({
        content:
          "❌ Error handling pulse command. Please check the bot logs for more details.",
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      await interaction.editReply({
        content:
          "❌ Error starting server. Please check the bot logs for more details.",
      });
    }
    ...
    try {
      ...
      const actionRows = servers.map((server) => {
        const startButton = new ButtonBuilder()
          ...
          .setEmoji(server?.state === ServerState.RUNNING ? "🔄" : "▶️");
      ...
      const collector = interaction.channel?.createMessageComponentCollector({
        filter: (i) =>
          (i.customId.startsWith("list-start-") ||
            i.customId.startsWith("list-stop-")) &&
          i.user.id === interaction.user.id,
        time: 300000, // 5 minutes
      });
      ...
      collector?.on("collect", async (buttonInteraction) => {
        ...
        try {
          ...
          console.error(
            `Error ${isStart ? "starting" : "stopping"} server:`,
            error,
          );
          await progressMessage.edit({
      ...
      await interaction.editReply(
        "❌ Error listing servers. Please check the bot logs for more details.",
      );
    }
  }
}
