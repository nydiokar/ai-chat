...
export const taskCommands = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand.setName("stats").setDescription("Show task statistics"),
  )
  .addSubcommand((subcommand) =>
    subcommand.setName("help").setDescription("Show task commands help"),
  );
...
async function ensureUserExists(
  ...
  try {
    ...
    if (!existingUser) {
      console.log(
        `Creating user record for Discord user ${userId} (${username})`,
      );
  ...
  try {
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "create": {
        ...
        const priority =
          (interaction.options.getString("priority") as TaskPriority) ||
          TaskPriority.MEDIUM;
          ...
          if (createdTasks.length > 0) {
            ...
            createdTasks.forEach((task: any) => {
              ...
              if (task.dueDate) {
                ...
                const now = new Date();
                const isOverdue =
                  dueDate < now && task.status !== TaskStatus.COMPLETED;
      ...
      case "stats": {
        const performanceMonitoring = await import(
          "../../../services/performance/performance-monitoring.service.js"
        );
        const metrics =
          await performanceMonitoring.PerformanceMonitoringService.getInstance().generatePerformanceDashboard();
        ...
        Object.entries(taskMetrics.tasksPerStatus).forEach(
          ([status, count]) => {
        ...
        Object.entries(taskMetrics.tasksByPriority).forEach(
          ([priority, count]) => {
