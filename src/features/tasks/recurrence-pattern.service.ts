    ...
    switch (pattern.type) {
      ...
      case RecurrenceType.WEEKLY: {
        ...
        if (nextDayThisWeek !== undefined) {
          ...
          nextDate.setUTCDate(
            nextDate.getUTCDate() + (nextDayThisWeek - currentDayOfWeek),
          );
        } else {
          // Move to the earliest day next week
          nextDate.setUTCDate(
            nextDate.getUTCDate() + (7 - currentDayOfWeek) + sortedDays[0],
          );
        }
      ...
      case RecurrenceType.MONTHLY: {
        ...
        if (pattern.interval > 1) {
          ...
          const yearIncrement = Math.floor(
            (nextDate.getUTCMonth() + extraMonths) / 12,
          );

          nextDate.setUTCFullYear(
            nextDate.getUTCFullYear() + yearIncrement,
            newMonth,
            pattern.dayOfMonth,
          );
        }
