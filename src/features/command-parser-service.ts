import { TaskStatus } from "./tasks/types/task.js";
...
export class CommandParserService {
  ...
  parse(input: string): ParsedCommand {
    ...
    const htMatch = input.match(
      /^(?:ht|hot tokens?|tokens?)\s+(\w+)(?:\s+(.+))?$/i,
    );
    if (htMatch) {
    ...
    if (words.some((w) => ["task", "todo", "work", "item"].includes(w))) {
      ...
      if (this.containsAny(words, ["create", "add", "make", "new"])) {
        throw new CommandParserError(
          'To create a task, use format: create task "title" "description"',
        );
      }
      if (this.containsAny(words, ["show", "view", "display", "get"])) {
        throw new CommandParserError(
          "To view a task, use format: view task #id",
        );
      }
      if (this.containsAny(words, ["update", "change", "modify"])) {
        throw new CommandParserError(
          "To update a task, use format: update task #id status to [status]",
        );
      }
      if (this.containsAny(words, ["assign", "give"])) {
        throw new CommandParserError(
          "To assign a task, use format: assign task #id to @user",
        );
      }
      if (this.containsAny(words, ["list", "all"])) {
        throw new CommandParserError(
          "To list tasks, use format: list tasks or list [status] tasks",
        );
      }
      if (this.containsAny(words, ["delete", "remove"])) {
        throw new CommandParserError(
          "To delete a task, use format: delete task #id",
        );
      }
    }
