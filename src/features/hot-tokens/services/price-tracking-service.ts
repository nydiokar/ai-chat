...
export class PriceTrackingService {
      ...
      this.requestQueue.push(async () => {
        try {
          ...
          if (timeSinceLastRequest < this.RATE_LIMIT_DELAY) {
            await new Promise((r) =>
              setTimeout(r, this.RATE_LIMIT_DELAY - timeSinceLastRequest),
            );
          }
  ...
  async getTokenPrice(contractAddress: string): Promise<TokenPrice | null> {
    try {
      ...
      for (const url of urls) {
        ...
        if (!response.ok) {
          console.error(
            `DexScreener API error for ${url}: ${response.status} ${response.statusText}`,
          );
          continue;
        }
        ...
        if (url.includes("/search")) {
          if (!data.pairs || data.pairs.length === 0) {
            console.warn(
              `No search results found for token ${contractAddress}`,
            );
            continue;
          }
          ...
          if (solanaPairs.length === 0) {
            console.warn(
              `No Solana pairs found in search results for ${contractAddress}`,
            );
            continue;
          }

          console.log(
            `Found ${solanaPairs.length} Solana pairs for ${contractAddress} using search`,
          );
          return await this.processTokenPriceData(
            { pairs: solanaPairs },
            contractAddress,
          );
          ...
          if (!data.pairs || data.pairs.length === 0) {
            console.warn(
              `No pairs found for token ${contractAddress} using ${url}`,
            );
            continue;
          }

          console.log(
            `Found token ${contractAddress} with ${data.pairs.length} pairs using ${url}`,
          );
  ...
  private async processTokenPriceData(
    data: any,
    contractAddress: string,
    ...
    try {
        ...
        try {
          ...
          if (token) {
            ...
            if (token.marketCapFirstEntry === null) {
              console.log(
                `Setting initial market cap for ${contractAddress} to ${marketCap}`,
              );
              ...
              console.log(
                `Updating current market cap for ${contractAddress} from ${token.marketCapNow} to ${marketCap}`,
              );
              await this.prisma.hotToken.update({
                where: { contractAddress },
            ...
            console.warn(
              `Token ${contractAddress} not found in database, cannot update market cap`,
            );
          }
        } catch (error) {
      ...
      console.log(
        `Extracted profile from pair data:`,
        JSON.stringify(profile).substring(0, 200) + "...",
      );

      return profile;
    } catch (error) {
      console.error(
        `Error processing token price data for ${contractAddress}:`,
        error,
      );
      return null;
    }
  ...
  private extractLinksFromPair(
    ...
    if (pair.info?.socials && Array.isArray(pair.info.socials)) {
      console.log(
        `Found ${pair.info.socials.length} social media links in pair data`,
      );
      ...
      pair.info.socials.forEach((social: any, index: number) => {
        ...
        if (social.platform && social.handle) {
          ...
          switch (social.platform.toLowerCase()) {
            ...
            case "x":
              url = social.handle.startsWith("http")
                ...
                : social.handle.startsWith("@")
                  ? `https://twitter.com/${social.handle.substring(1)}`
            ...
            case "telegram":
              url = social.handle.startsWith("http")
                ? social.handle
                : social.handle.startsWith("@")
                  ? `https://t.me/${social.handle.substring(1)}`
            ...
            case "discord":
              url = social.handle.startsWith("http")
                ? social.handle
                : social.handle.startsWith("https://discord.gg/")
                  ? social.handle
            ...
            case "medium":
              url = social.handle.startsWith("http")
                ? social.handle
                : `https://medium.com/${social.handle}`;
              label = "Medium";
              break;
            case "github":
              url = social.handle.startsWith("http")
                ? social.handle
                : `https://github.com/${social.handle}`;
              label = "GitHub";
              break;
            default:
          ...
          console.log(
            `Converted social media: ${social.platform} -> ${label}: ${url}`,
          );
  ...
  async getTrendingTokens(
    ...
    try {
      console.log(
        `Fetching trending tokens for ${chainId} with timeframe ${timeframe}`,
      );

      // Map our timeframe format to DexScreener's format
      const dexScreenerTimeframe =
        timeframe === "1h"
          ? "h1"
          : timeframe === "24h"
            ? "h24"
  ...
  createPriceEmbed(price: TokenPrice | null): EmbedBuilder {
    ...
    embed.addFields(
      {
        name: "Price",
        value: `$${price.currentPrice.toFixed(8)}`,
        inline: true,
      },
      {
        name: "Liquidity",
        value: `$${price.liquidity.toLocaleString()}`,
        inline: true,
      },
      {
        name: "24h Volume",
        value: `$${price.volume24h.toLocaleString()}`,
        inline: true,
      },
      {
        name: "Price Changes",
      ...
      {
        name: "Market Cap",
        value: `$${price.marketCap.toLocaleString()}`,
        inline: true,
      },
      {
        name: "DEX Info",
  ...
  private async triggerAlert(
    ...
    try {
      ...
      console.log(
        `Alert triggered for ${price.name} at $${price.currentPrice}`,
      );
    } catch (error) {
    ...
    try {
      ...
      for (const token of knownTokens) {
        ...
        for (const url of urls) {
          ...
          if (!response.ok) {
            console.error(
              `DexScreener API error for known token: ${response.status} ${response.statusText}`,
            );
            continue;
          }
          ...
          const data = await response.json();
          console.log(
            `API response for known token:`,
            JSON.stringify(data).substring(0, 200) + "...",
          );

          if (data.pairs && data.pairs.length > 0) {
            console.log(
              `Found ${data.pairs.length} pairs for known token using ${url}`,
            );
            console.log(
              `First pair data:`,
              JSON.stringify(data.pairs[0]).substring(0, 200) + "...",
            );
