...
export class HotTokensService {
  ...
  async setPriceAlert(
    ...
    return this.priceTrackingService.setPriceAlert(
      contractAddress,
      targetPrice,
      condition,
      userId,
  ...
  async createTrendingEmbed(
    tokens: TokenPrice[],
    isWatchlist: boolean = false,
    chainId?: string,
  ) {
    const title = isWatchlist
      ? "🔥 Top Performing Watchlist Tokens"
      : "🔥 Trending Tokens";
    ...
    return this.priceTrackingService.createTrendingEmbed(
      tokens,
      title,
      description,
    );
  ...
  async createListEmbed(tokens: HotToken[]): Promise<EmbedBuilder> {
    ...
    if (tokens.length === 0) {
      embed.addFields({
        ...
        value: "Add some tokens with `/ht add`!",
      });
    } else {
      ...
      for (const [index, token] of tokens.entries()) {
        ...
        if (
          token.marketCapNow &&
          token.marketCapFirstEntry &&
          token.marketCapFirstEntry > 0
        ) {
          const growth =
            ((token.marketCapNow - token.marketCapFirstEntry) /
              token.marketCapFirstEntry) *
            100;
        ...
        if (price?.links) {
          for (const link of price.links as Array<{
            type: string;
            url: string;
          }>) {
        ...
        const content =
          `**${index + 1}. ${categoryEmoji} ${token.name} ${price?.symbol ? `($${price.symbol})` : ""}**` +
