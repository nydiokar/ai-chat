...
export const hotTokensCommands = new SlashCommandBuilder()
    ...
    subcommand
      ...
      .addStringOption((option) =>
        option
          .setName("name")
          .setDescription(
            "Token name (optional, will auto-fetch if not provided)",
          )
          .setRequired(false),
      )
      .addStringOption((option) =>
      ...
      .addStringOption((option) =>
        option
          .setName("tags")
          .setDescription(
            'Comma-separated list of tags (e.g., "trending,new,solana")',
          )
          .setRequired(false),
      ),
  )
  ...
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .addStringOption((option) =>
        option.setName("note").setDescription("New note").setRequired(false),
      )
      .addStringOption((option) =>
        option
          .setName("tags")
    ...
    subcommand
      ...
      .addBooleanOption((option) =>
        option
          .setName("watchlist")
          .setDescription(
            "Show top performing tokens from your watchlist instead",
          )
          .setRequired(false),
      ),
  )
...
export async function handleHotTokensCommand(
  interaction: ChatInputCommandInteraction,
  hotTokensService: HotTokensService,
) {
  try {
    ...
    switch (subcommand) {
      ...
      case "add": {
        ...
        const category =
          (interaction.options.getString("category") as TokenCategory) ||
          "UNKNOWN";
        ...
        try {
          ...
          const priceData =
            await priceTrackingService.getTokenPrice(contractAddress);

          if (!priceData && !providedName) {
            await interaction.editReply(
              `❌ Could not fetch token data and no name was provided. Please provide a name or try again later.`,
            );
            return;
          }
          ...
          await interaction.editReply(
            `❌ Failed to add token: ${(error as Error).message}`,
          );
        }
        break;
      }
      ...
      case "update": {
        ...
        const category = (interaction.options.getString("category") ||
          undefined) as TokenCategory | undefined;
        ...
        const tags = tagsString
          ...
          : undefined;
        const isCommunity =
          interaction.options.getBoolean("community") ?? undefined;
        ...
        const updated = await hotTokensService.updateToken(
          contractAddress,
          update,
        );
      ...
      case "list": {
        ...
        if (tokens.length === 0) {
          await interaction.editReply(
            "No tokens found in the hot list with the specified filters.",
          );
          return;
        }
      ...
      case "price": {
        ...
        if (!priceData) {
          await interaction.editReply(
            "❌ Unable to fetch price data for the token.",
          );
          return;
        }
        ...
        embed.addFields(
          {
            name: "Price",
            value: `$${priceData.currentPrice.toFixed(8)}`,
            inline: true,
          },
          {
            name: "Liquidity",
            value: `$${priceData.liquidity.toLocaleString()}`,
            inline: true,
          },
          {
            name: "24h Volume",
        ...
        if (priceData.url) {
          ...
          links.push(
            `[📊 Chart](https://dexscreener.com/${priceData.chainId}/${priceData.pairAddress})`,
          );
        }
        ...
        embed.setFooter({
          text: `Data from DexScreener • Today at ${new Date().toLocaleTimeString(
            "en-US",
            {
              hour: "2-digit",
              minute: "2-digit",
      ...
      case "trending": {
        const timeframe =
          (interaction.options.getString("timeframe") as
            | "1h"
            | "24h"
            | "7d"
            | "30d") || "24h";
        const chain =
          (interaction.options.getString("chain") as string) || "solana";
        const useWatchlist =
          interaction.options.getBoolean("watchlist") ?? false;
        ...
        if (useWatchlist) {
          ...
          topTokens = await hotTokensService.getTrendingTokens(
            chain,
            timeframe,
          );
        }
        ...
        const embed = await hotTokensService.createTrendingEmbed(
          topTokens,
          useWatchlist,
          useWatchlist ? undefined : chain,
        );
      ...
      case "alert": {
        ...
        const alert = await hotTokensService.setPriceAlert(
          contractAddress,
          targetPrice,
          condition,
          interaction.user.id,
