...
import {
  DiscordContext,
  DiscordCachedMessage,
  DiscordCachedConversation,
} from "../types/discord.js";
import {
  hotTokensCommands,
  handleHotTokensCommand,
} from "../features/hot-tokens/commands/hot-tokens-commands.js";
import {
  taskCommands,
  handleTaskCommand,
} from "../features/tasks/commands/task-commands.js";
import {
  pulseCommand,
  handlePulseCommand,
} from "../features/pulse-mcp/commands/pulse-discord-command.js";
...
export class DiscordService {
  ...
  private async initializeCache(): Promise<void> {
    try {
      ...
      logError(
        ...
        createLogContext("DiscordService", "initializeCache", {
          error: err.message,
        }),
      );
      throw err;
  ...
  private async getAgent(): Promise<Agent> {
    if (!this.agent) {
      ...
      this.agent = await AIFactory.create(
        defaultConfig.openai.model as AIModel,
        undefined,
        memoryProvider,
      );
    }
  ...
  private async handleMessage(message: Message): Promise<void> {
    ...
    if (allowedGuildId && message.guild?.id !== allowedGuildId) {
      debug(
        `Message ignored - not from allowed guild. Message guild: ${message.guild?.id}, Allowed guild: ${allowedGuildId}`,
      );
      return;
    }
    ...
    if (!isDM && !isBotMentioned) {
      debug(
        `Message ignored - bot not mentioned: ${message.content.substring(0, 50)}...`,
      );
      return;
    }
    ...
    if (isBotMentioned) {
      ...
      cleanContent = cleanContent
        ...
        .trim();
      debug(
        `Processing message with bot mention: ${cleanContent.substring(0, 50)}...`,
      );
    }
    ...
    if (cleanContent.toLowerCase().startsWith("debug")) {
      ...
      if (args[1]?.toLowerCase() === "on") {
        ...
        if (service && service instanceof ReActAgent) {
          ...
          await message.reply(
            "Debug mode enabled - showing full thought process",
          );
        }
        return;
    ...
    if (
      cleanContent.match(
        /^(task|ht|help|list|create|view|update|assign|delete|stats)/i,
      )
    ) {
    ...
    try {
      ...
      try {
        const cachedSession =
          await this.sessionCache.get<DiscordCachedConversation>(sessionKey);
        if (cachedSession) {
          // Verify the cached conversation still exists in the database
          const dbConversation = await this.db.getConversation(
            cachedSession.id,
          );
          if (dbConversation) {
            ...
            debug(
              "Cached conversation not found in database, creating new one",
            );
          }
        }
      } catch (error) {
      ...
      if (!conversation) {
        ...
        if (!conversation) {
          ...
          if (!conversation) {
            throw new Error(
              `Failed to create conversation - could not verify conversation ${conversationId} exists`,
            );
          }
          debug(
            `Created and verified new conversation with ID: ${conversationId}`,
          );
        }
        ...
        try {
          await this.sessionCache.set(
            sessionKey,
            conversation as DiscordCachedConversation,
          );
        } catch (error) {
          debug(
            `Failed to cache conversation: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      ...
      const llmMessages = conversation.messages
        ...
        .map(
          (msg: DiscordCachedMessage): Input => ({
      ...
      try {
        ...
        debug(
          `Failed to update cache with new messages: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      if (timeSinceLastActivity >= this.contextRefreshInterval) {
        ...
        if (
          ...
          conversation.messages.length <=
            defaultConfig.messageHandling.maxContextMessages
        )
          return;
  ...
  private async summarizeConversation(conversation: {
    id: number;
    model: string;
    ...
    try {
      ...
      debug(
        `Error summarizing conversation: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  public async start(): Promise<void> {
    try {
      ...
      try {
        ...
        logError(
          "Failed to initialize MCP container and AIFactory, continuing without it",
          createLogContext("DiscordService", "start", {
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      logError(
        "Failed to start Discord service",
        createLogContext("DiscordService", "start", { error: err.message }),
      );
      throw err;
    }
  }
    ...
    try {
      ...
      if (guildId) {
        ...
        try {
          ...
          debug(
            `Successfully registered ${commands.length} commands for guild ${guildId}`,
          );
        } catch (error) {
          debug(
            "Guild registration failed, falling back to global registration",
          );
  ...
  static async getInstance(): Promise<DiscordService> {
    ...
    if (!DiscordService.instances.has(env)) {
      ...
      const instance = new DiscordService(token, DatabaseService.getInstance());
