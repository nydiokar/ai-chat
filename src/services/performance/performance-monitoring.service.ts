...
export class PerformanceMonitoringService {
  ...
  private async collectToolUsageStats() {
    try {
      return await this.dbService.executePrismaOperation(async (prisma) => {
        ...
        const allTools: ToolUsageStats[] = toolUsageRecords.reduce(
          (acc: ToolUsageStats[], record) => {
            // Validate and normalize status
            const validStatus = ["success", "failure", "error"].includes(
              record.status || "",
            )
              ? record.status
              : "unknown";
        ...
        allTools.forEach((tool) => {
          ...
          totalSuccessfulCalls += tool.usage.filter(
            (u) => u.status === "success",
          ).length;
        });
        ...
        const successRate =
          totalCalls > 0 ? totalSuccessfulCalls / totalCalls : 0;

        // Generate top tools statistics
        const topTools = allTools
          .map((tool) => {
            const totalTime = tool.usage.reduce(
            ...
            return {
              ...
              averageExecutionTime:
                tool.usage.length > 0 ? totalTime / tool.usage.length : 0,
            };
          })
  ...
  private async collectQueryPerformance() {
    ...
    return {
      ...
      averageQueryTime:
        queryMetrics.reduce((sum, metric) => sum + metric.executionTime, 0) /
        queryMetrics.length,
  ...
  private async collectTaskMetrics() {
    try {
      ...
      const tasksPerStatus = tasks.reduce(
        (acc, task) => {
  ...
  async generatePerformanceDashboard(): Promise<PerformanceMetrics> {
    ...
    if (metrics.queryPerformance.averageQueryTime > 0) {
      console.log(
        `Average query time: ${metrics.queryPerformance.averageQueryTime}ms`,
      );
    }
