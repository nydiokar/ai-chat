...
export class AIFactory {
  ...
  static async initialize(
    configOrContainer: MCPConfig | MCPContainer = mcpConfig,
  ): Promise<void> {
    if (this.container) {
      throw new MCPError(
    ...
    // Log initialization
    debug(
      "AIFactory initialized",
      createLogContext("AIFactory", "initialize", {
    ...
    try {
      ...
      if ("on" in this.toolManager) {
        (this.toolManager as any).on("tools.refreshed", () => {
          debug(
            "Tools refreshed by tool manager",
            createLogContext("AIFactory", "toolRefresh", {}),
          );
        });
      }
    } catch (err) {
      warn(
        "Some tools failed to load, attempting recovery",
        createLogContext("AIFactory", "initialize", {
      ...
      try {
        ...
        if (tools.length > 0) {
          ...
          warn(
            "No tools available after recovery attempt",
            createLogContext("AIFactory", "initialize", {}),
          );
        }
      } catch (toolError) {
        error(
          "Failed to get any tools",
          createErrorContext(
            "AIFactory",
            "initialize",
            "System",
  ...
  static async create(
    ...
    try {
      ...
      config.openai.model =
        selectedModel === "gpt" || selectedModel === "openai"
  ...
  static cleanup(): void {
    ...
    debug("AIFactory cleaned up", createLogContext("AIFactory", "cleanup", {}));
  }
