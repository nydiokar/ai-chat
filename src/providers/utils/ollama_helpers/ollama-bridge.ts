...
export class OllamaBridge {
  ...
  private async getRelevantToolsFromPromptGenerator(
    message: string,
  ...
  private async convertToOllamaRequest(
    request: OllamaChatRequest,
  ): Promise<ChatRequest & { stream?: false }> {
    const userMessage = request.messages[request.messages.length - 1].content;
    const relevantTools =
      await this.getRelevantToolsFromPromptGenerator(userMessage);

    // Get ReAct formatted prompt
    const systemPrompt = await this.promptGenerator.generatePrompt(
      userMessage,
      relevantTools,
    );
  ...
  private async getOllamaResponse(
    ...
    try {
      const response = (await ollama.chat(
        await this.convertToOllamaRequest(request),
      )) as any;
