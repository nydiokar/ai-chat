...
class MockMemoryProvider {
  ...
  async update() { return { id: 'mock-id' }; }
...
async function testReActTrace() {
  ...
  const finalResponse = "After analyzing the data, I found that recent AI advancements include significant improvements in natural language processing and computer vision.";
  trace.markComplete(finalResponse);
  
  console.log(`Is reasoning complete: ${trace.isReasoningComplete()}`);
  console.log(`Final response (truncated): ${trace.getFinalResponse().substring(0, 50)}...`);
