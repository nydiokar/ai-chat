...
function getAIProvider(): AIModel {
  ...
  if (!Object.values(AIProviders).includes(provider as any)) {
    warn(
      ...
      createLogContext(COMPONENT, "getAIProvider", {
        provider,
  ...
  info(
    ...
    createLogContext(COMPONENT, "getAIProvider", { provider }),
  );
...
function getOpenAIModel(currentProvider: AIModel): string {
  ...
  if (currentProvider !== AIProviders.OPENAI) {
    info(
      "Non-OpenAI provider, using default model",
      createLogContext(COMPONENT, "getOpenAIModel", {
  ...
  info(
    "Using default OpenAI model for environment",
    createLogContext(COMPONENT, "getOpenAIModel", {
...
export interface BaseConfig {
  ...
  discord: {
    ...
    mcp: {
      // MCP under Discord since it should only be used there
      enabled: boolean;
      authToken?: string;
...
export const defaultConfig: BaseConfig = {
  ...
  discord: {
    ...
    mcp: {
      ...
      logLevel: (process.env.MCP_LOG_LEVEL || "info") as
        | "error"
        | "warn"
        | "info"
        | "debug",
...
export function validateEnvironment(): void {
  ...
  if (model === "ollama") {
    process.env.OLLAMA_HOST =
      process.env.OLLAMA_HOST || "http://127.0.0.1:11434"; // Explicitly use IPv4
    process.env.OLLAMA_MODEL = process.env.OLLAMA_MODEL || "llama3.2.latest";
    info(
      "Configured Ollama settings",
      createLogContext(COMPONENT, "validateEnvironment", {
  ...
  if (process.env.DISCORD_ENABLED === "true") {
    ...
    if (process.env.MCP_ENABLED === "true") {
      ...
      if (
        ...
        !validLogLevels.includes(process.env.MCP_LOG_LEVEL)
      ) {
        error(
          "Invalid MCP log level",
          createErrorContext(
            COMPONENT,
            "validateEnvironment",
            "System",
            "CONFIG_ERROR",
            new Error(
              `Invalid MCP_LOG_LEVEL. Must be one of: ${validLogLevels.join(", ")}`,
            ),
...
export function validateInput(
  ...
  if (!sanitized) {
    warn(
      ...
      createLogContext(COMPONENT, "validateInput", {
