...
describe("Task-Memory Integration", () => {
  ...
  describe("Task Creation with Memory Context", () => {
    it("should create a task and its conversation context", async () => {
      ...
      const contexts =
        await memoryRepo.getContextByConversation(testConversationId);
  ...
  describe("Task Dependencies and Memory", () => {
    ...
    it("should maintain memory context across dependent tasks", async () => {
      ...
      expect(
        result.some((r) =>
          (r.data as ConversationContext).topics.includes("parent-task"),
        ),
      ).to.be.true;
      expect(
        result.some((r) =>
          (r.data as ConversationContext).topics.includes("child-task"),
        ),
      ).to.be.true;
    });
    ...
    it("should propagate status changes through dependencies", async () => {
      ...
      expect((result[0].data as ConversationContext).entities).to.include(
        "task-completion",
      );
    });
  });
  ...
  describe("Memory Cleanup", () => {
    it("should clean up memory contexts when task is deleted", async () => {
      ...
      expect(result).to.have.length(
        0,
        "Expected all task contexts to be cleaned up",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle concurrent operation failures gracefully", async () => {
      ...
      try {
        ...
        expect(errorHandlingContexts).to.have.length(
          1,
          "Expected error handling context to be saved",
        );
      }
    });
  });
