...
const testServer: PulseMCPServer = {
  ...
  url: TEST_REPO_URL,
  short_description:
    "Enhanced MCP server for deep web research with intelligent search queuing and content extraction",
  ...
  try {
    ...
    try {
      ...
      if (missingFiles.length > 0) {
        console.error(
          `Missing required files in src: ${missingFiles.join(", ")}`,
        );
        return false;
      }
...
async function testGitHubPreparation(): Promise<boolean> {
  ...
  try {
    ...
    // Get repository path
    const repoName =
      testServer.source_code_url.split("/").pop()?.replace(".git", "") || "";
    ...
    if (!entryPoint.startsWith(REPOS_DIR)) {
      console.error(
        `Entry point ${entryPoint} is not within mcp-repos directory`,
      );
      return false;
    }
...
`;
    ...
    const newConfig =
      configContent.slice(
        0,
        insertPosition + "// DYNAMICALLY ADDED SERVERS".length,
      ) +
      "\n" +
      serverEntry +
      "\n" +
...
async function testStartingServer(): Promise<boolean> {
  ...
  try {
    ...
    if (server.state === ServerState.RUNNING) {
      ...
      console.error(
        `Server did not start successfully. State: ${server.state}`,
      );
      return false;
    }
  } catch (error) {
