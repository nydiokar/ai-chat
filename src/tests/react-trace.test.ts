...
class MockMemoryProvider implements MemoryProvider {
  ...
  async initialize(): Promise<void> { /* Not used in tests */ }
  async store(): Promise<any> { /* Not used in tests */ return { id: 'mock-id' }; }
  async storeThoughtProcess(step: ReasoningStep, userId: string, metadata?: Record<string, any>): Promise<any> {
    return this.storeThoughtProcessStub(step, userId, metadata);
  }
  async search(): Promise<any> {
    return this.searchStub();
  }
  async getById(): Promise<any> { /* Not used in tests */ return null; }
  async update(): Promise<any> { /* Not used in tests */ return { id: 'mock-id' }; }
  async delete(): Promise<boolean> { /* Not used in tests */ return true; }
  async getSummary(): Promise<string> { /* Not used in tests */ return ''; }
  async clearUserMemories(): Promise<void> { /* Not used in tests */ }
  async getRelevantMemories(): Promise<any[]> { /* Not used in tests */ return []; }
  async cleanup(): Promise<void> { /* Not used in tests */ }
}
...
describe('ReActTrace', () => {
  ...
  beforeEach(() => {
    ...
    trace = new ReActTrace(memoryProviderMock as unknown as MemoryProvider, userId);
  });
  ...
  it('should add steps to the trace', async () => {
    ...
    expect(memoryProviderMock.storeThoughtProcessStub.firstCall.args[0]).to.deep.equal(step);
    expect(memoryProviderMock.storeThoughtProcessStub.firstCall.args[1]).to.equal(userId);
    expect(memoryProviderMock.storeThoughtProcessStub.firstCall.args[2]).to.have.property('sessionId');
  });
  ...
  it('should optimize steps correctly', async () => {
    // Create 5 steps
    const steps: ReasoningStep[] = Array(5).fill(0).map((_, i) => ({
      stepId: `step${i+1}`,
      timestamp: new Date().toISOString(),
      thought: {
  ...
  it('should extract topics from steps', async () => {
    const step: ReasoningStep = {
      ...
      timestamp: new Date().toISOString(),
      thought: {
        reasoning: 'I need to search for information about artificial intelligence techniques',
