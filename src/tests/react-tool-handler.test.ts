...
describe('ReActToolHandler', () => {
  ...
  beforeEach(() => {
    ...
    toolHandler = new ReActToolHandler(
      mockToolManager,
      mockToolExecutor
    );
  });
  ...
  it('should create tool execution functions that call the tool manager', async () => {
    ...
    sinon.assert.calledWith(mockToolManager.executeTool as sinon.SinonStub, 'web_search', { query: 'test query' });
  ...
  it('should execute tools with registry using tool executor', async () => {
    ...
    const result = await toolHandler.executeToolWithRegistry(action, registry, 'user123');
