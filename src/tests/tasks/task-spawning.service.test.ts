...
describe("TaskSpawningService", () => {
  ...
  describe("spawnTaskInstance", () => {
    it("should create a new task instance with correct properties", async () => {
      ...
      // Call the method
      const result = await taskSpawningService.spawnTaskInstance(
        parentTask,
        occurrenceDate,
      );
      ...
      // Assertions
      expect(
        taskRepositoryStub.createTask.calledOnceWith(
          sinon.match(expectedTaskInstance),
        ),
      ).to.be.true;
  ...
  describe("processRecurringTasks", () => {
    it("should spawn tasks for due recurring tasks", async () => {
      ...
      sinon
        .stub(RecurrencePatternService, "getNextOccurrence")
        .returns(new Date(2025, 2, 1)); // March 1st, 2025
      ...
      taskRepositoryStub.listTasks
        .withArgs(sinon.match({ parentTaskId: 1 }))
        .resolves({
          tasks: [],
          total: 0,
        });
      ...
      const spawnStub = sinon
        .stub(taskSpawningService, "spawnTaskInstance")
        .resolves();
      ...
      expect(
        (RecurrencePatternService.shouldSpawnTask as sinon.SinonStub)
          .calledOnce,
      ).to.be.true;
      expect(
        (RecurrencePatternService.getNextOccurrence as sinon.SinonStub)
          .calledOnce,
      ).to.be.true;
    });
  });
  ...
  describe("completeTaskInstance", () => {
    it("should complete a task and log history", async () => {
      ...
      // Call the method
      const result = await taskSpawningService.completeTaskInstance(
        taskId,
        userId,
      );
      ...
      // Assertions
      expect(
        taskRepositoryStub.updateTask.calledOnceWith(taskId, {
          status: TaskStatus.COMPLETED,
        }),
      ).to.be.true;
      expect(taskRepositoryStub.addTaskHistory.calledOnce).to.be.true;
