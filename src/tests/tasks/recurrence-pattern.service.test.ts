...
describe("RecurrencePatternService", () => {
  ...
  describe("getNextOccurrence", () => {
    it("should calculate next custom occurrence - every minute", () => {
      ...
      expect(nextOccurrence?.toISOString()).to.equal(
    ...
    it("should calculate next custom occurrence - specific time", () => {
      ...
      expect(nextOccurrence?.toISOString()).to.equal(
        new Date(Date.UTC(2025, 1, 1, 9, 0, 0)).toISOString(),
      );
    });
    ...
    it("should calculate next custom occurrence - weekdays only", () => {
      ...
      const sundayOccurrence = RecurrencePatternService.getNextOccurrence(
        pattern,
        new Date(Date.UTC(2025, 1, 2, 8, 0, 0)),
      );
      expect(sundayOccurrence?.toISOString()).to.equal(
        new Date(Date.UTC(2025, 1, 3, 9, 0, 0)).toISOString(),
      ); // Should be Monday

      // Starting from Friday after work hours
      const fridayOccurrence = RecurrencePatternService.getNextOccurrence(
        pattern,
        new Date(Date.UTC(2025, 1, 7, 18, 0, 0)),
      );
      expect(fridayOccurrence?.toISOString()).to.equal(
        new Date(Date.UTC(2025, 1, 10, 9, 0, 0)).toISOString(),
    ...
    it("should calculate next custom occurrence - with intervals", () => {
      ...
      expect(nextOccurrence?.toISOString()).to.equal(
        new Date(Date.UTC(2025, 1, 1, 10, 15, 0)).toISOString(),
      );
    });
    ...
    it("should calculate next custom occurrence - with lists", () => {
      ...
      expect(nextOccurrence?.toISOString()).to.equal(
        new Date(Date.UTC(2025, 1, 1, 10, 30, 0)).toISOString(),
      );
    });
    ...
    it("should calculate next daily occurrence", () => {
      ...
      expect(nextOccurrence?.toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 2)).toDateString(),
      );
    });
    ...
    it("should calculate next weekly occurrence", () => {
      ...
      expect(nextOccurrence?.toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 3)).toDateString(),
      ); // Should be Monday

      // Starting from Friday (5)
      const fridayOccurrence = RecurrencePatternService.getNextOccurrence(
        pattern,
        new Date(Date.UTC(2025, 1, 7)),
      );
      expect(fridayOccurrence?.toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 10)).toDateString(),
      ); // Should be next Monday
    });
    ...
    it("should calculate next monthly occurrence", () => {
      ...
      expect(nextOccurrence?.toDateString()).to.equal(
        new Date(Date.UTC(2025, 2, 15)).toDateString(),
      );
    });
    ...
    it("should handle end conditions", () => {
      ...
      const pastEndDate = RecurrencePatternService.getNextOccurrence(
        pattern,
  ...
  describe("shouldSpawnTask", () => {
    it("should determine if task should spawn based on current time", () => {
      ...
      expect(RecurrencePatternService.shouldSpawnTask(pattern, yesterday)).to.be
        .true;

      const tomorrow = new Date(Date.now() + 86400000);
      expect(RecurrencePatternService.shouldSpawnTask(pattern, tomorrow)).to.be
        .false;
    });
  });
  ...
  describe("getOccurrences", () => {
    it("should get multiple occurrences up to max count", () => {
      ...
      const occurrences = RecurrencePatternService.getOccurrences(
        pattern,
        startDate,
        3,
      );
      expect(occurrences).to.have.length(3);
      expect(occurrences[0].toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 2)).toDateString(),
      );
      expect(occurrences[1].toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 3)).toDateString(),
      );
      expect(occurrences[2].toDateString()).to.equal(
        new Date(Date.UTC(2025, 1, 4)).toDateString(),
      );
    });
