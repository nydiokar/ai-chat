...
describe("QueryOptimizationService", () => {
  ...
  describe("getCachedResult", () => {
    it("should return cached result if available and not expired", async () => {
      ...
      const result1 = await queryOptimizationService.getCachedResult(
        queryHash,
        queryFn,
      );
      ...
      const result2 = await queryOptimizationService.getCachedResult(
        queryHash,
        queryFn,
      );
  ...
  describe("getQueryMetrics", () => {
    it("should return query metrics within date range", async () => {
      ...
      (dbServiceStub.prisma.queryMetrics.findMany as sinon.SinonStub).resolves(
        testMetrics,
      );
  ...
  describe("getCacheAnalytics", () => {
    it("should calculate correct cache statistics", async () => {
      ...
      // Update the existing mock's findMany method
      (dbServiceStub.prisma.cacheMetrics.findMany as sinon.SinonStub).resolves(
        cacheMetrics,
      );
