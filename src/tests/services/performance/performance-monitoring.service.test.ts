...
describe("PerformanceMonitoringService", () => {
  ...
  beforeEach(() => {
    ...
    (dbServiceStub.executePrismaOperation as sinon.SinonStub).callsFake(
      async <T>(operation: (prisma: any) => Promise<T>): Promise<T> => {
        const mockTx = {
          toolUsage: {
            findMany: sinon.stub().resolves([]),
          },
  ...
  describe("generatePerformanceDashboard", () => {
    it("should collect and store system metrics", async () => {
      ...
      expect(metrics.toolUsageStats)
        .to.have.property("totalToolCalls")
        .that.equals(2);
      expect(metrics.queryPerformance)
        .to.have.property("totalQueries")
        .that.equals(2);
  ...
  describe("setupPeriodicMonitoring", () => {
    ...
    it("should setup periodic monitoring at specified interval", async () => {
      const generateDashboardStub = sinon
        .stub(performanceService, "generatePerformanceDashboard")
        .resolves();
