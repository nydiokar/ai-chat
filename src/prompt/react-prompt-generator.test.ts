...
describe("ReActPromptGenerator", () => {
  ...
  describe("generateReActPrompt", () => {
    ...
    it("should include appropriate guidance based on step count", async () => {
      ...
      const initialPrompt = await promptGenerator.generateReActPrompt(
        "test query",
        [],
        mockTools,
      );
      ...
      console.log("\n=== DEBUGGING ===");
      console.log(
        "Steps has observation:",
        stepsWithObservation.some((s) => s.observation !== undefined),
      );

      // Test with steps that include an observation (follow-up prompt)
      const followUpPrompt = await promptGenerator.generateReActPrompt(
        "test query",
        stepsWithObservation,
        mockTools,
      );
      console.log(
        "Guidance section of prompt:",
        followUpPrompt.split("\n\n").pop(),
      );
      console.log("Full follow-up prompt:", followUpPrompt);
  ...
  describe("tool prioritization", () => {
    it("should prioritize search tools for search queries", async () => {
      ...
      const tools = await promptGenerator.getTools(
  ...
  describe("scratchpad injection", () => {
    it("injects scratchpad summary as Task state block when provided", async () => {
      ...
      expect(prompt).to.include("Task state:");
      expect(prompt).to.include("Goal: find coffee");
      expect(prompt).to.include("Found 3 cafes nearby");
    });
  ...
  describe("error handling", () => {
    it("should provide a fallback prompt on error", async () => {
      ...
      sandbox
        .stub((promptGenerator as any).toolFormatter, "formatToolDescriptions")
