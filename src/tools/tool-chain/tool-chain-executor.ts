  ...
  async execute(
    ...
    try {
      ...
      for (const tool of chainConfig.tools) {
        if (
          this.shouldAbortChain(chainConfig, executionContext, chainResults)
        ) {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => {
            reject(
              new Error(`TIMEOUT: Tool execution timeout after ${timeoutMs}ms`),
            );
          }, timeoutMs);
        });
        ...
        if (typeof value === "string" && value.startsWith("$")) {
          ...
          if (contextValue === undefined) {
            return {
              success: false,
              error: new Error(
                `Missing context value for parameter ${key}: ${value}`,
              ),
            };
          }
          ...
          for (let i = 1; i < pathSegments.length; i++) {
            if (contextValue === undefined) {
              return {
                success: false,
                error: new Error(
                  `Cannot access ${pathSegments[i]} of undefined in path ${value}`,
                ),
              };
            }
