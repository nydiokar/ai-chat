...
describe("ToolChainExecutor", function () {
  ...
  it("handles errors in the middle of chain", async () => {
    ...
    expect(registry.first.calledOnce, "first tool should be called once").to.be
      .true;
    expect(
      registry.failingTool.callCount,
      "failing tool should be called 4 times (1 attempt + 3 retries)",
    ).to.equal(4);
    expect(registry.third.called, "third tool should not be called").to.be
      .false;
