...
export class BaseServerManager extends EventEmitter implements IServerManager {
  ...
  protected appendError(id: string, error: Error | string): void {
    ...
    if (server.lastError) {
      ...
      const previousError =
        server.lastError.message || String(server.lastError);
  ...
  protected setupNotificationHandlers(id: string, client: IMCPClient): void {
    ...
    console.log(
      `Server manager is aware of server ${id}, but notifications are handled by the client`,
    );
  }
    ...
    try {
      ...
      if (server.client) {
        ...
        try {
          ...
          console.error(
            `Error reusing client for ${id}, will create new one: ${error}`,
          );
          ...
          try {
            ...
            console.warn(
              `Error disconnecting client for ${id}: ${disconnectError}`,
            );
          }
          ...
          if (clientId && this.container.isBound(clientId)) {
            try {
              ...
              console.log(
                `Unbound client ${clientId} from container for clean restart`,
              );
            } catch (unbindError) {
              console.warn(
                `Error unbinding client ${clientId}: ${unbindError}`,
              );
            }
          }
        }
    ...
    try {
      ...
      if (clientId && this.container.isBound(clientId)) {
        try {
          ...
          console.error(
            `Error disconnecting client for server ${id}:`,
            disconnectError,
          );
      ...
      if (server.client) {
        try {
          await server.client.disconnect();
          console.log(
            `Successfully disconnected direct client reference for server ${id}`,
          );
        } catch (directDisconnectError) {
          console.error(
            `Error disconnecting direct client for server ${id}:`,
            directDisconnectError,
          );
          // Continue with server stop even if disconnect fails
        }
  ...
  public async restartServer(serverId: string): Promise<void> {
    ...
    try {
      ...
      this.emit("server.state.changed", {
        id: serverId,
      ...
      try {
        ...
        console.warn(
          `Warning: Error stopping server ${serverId} during restart, continuing anyway:`,
          stopError,
        );
      ...
      if (server.client) {
        try {
          console.log(
            `Forcefully disconnecting client for ${serverId} before restart`,
          );

          // Try to disconnect at the transport level
          if (
            (server.client as any)._transport &&
            typeof (server.client as any)._transport.stop === "function"
          ) {
            try {
              ...
              console.warn(
                `Warning: Error stopping transport for ${serverId}:`,
                transportError,
              );
            }
          }
          ...
          console.warn(
            `Warning: Error during client cleanup for ${serverId}:`,
            cleanupError,
          );
        }
      }
