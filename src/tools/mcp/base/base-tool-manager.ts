...
export class BaseToolManager implements IToolManager {
  ...
  public async executeTool(name: string, args: any): Promise<ToolResponse> {
    this.logger.info(
      ...
      createLogContext("ToolManager", "executeTool", {
        toolName: name,
    ...
    if (!tool) {
      ...
      this.logger.warn(
        "Tool not found",
        createLogContext("ToolManager", "executeTool", { toolName: name }),
      );
      return {
        success: false,
        data: null,
    ...
    if (handler) {
      try {
        ...
        this.logger.info(
          "Local handler execution completed",
          createLogContext("ToolManager", "executeTool", {
            toolName: name,
        ...
        this.logger.error(
          "Local handler execution failed",
          createErrorContext(
            "ToolManager",
            "executeTool",
            "System",
    ...
    try {
      ...
      if (!result.success && !result.error) {
        ...
        this.logger.warn(
          "Tool returned unsuccessful result without error details",
          createLogContext("ToolManager", "executeTool", {
            toolName: name,
            serverId: tool.server?.id || "unknown",
          }),
  ...
  public async refreshToolInformation(force: boolean = false): Promise<void> {
    ...
    for (const [serverId, summary] of Object.entries(toolSummary)) {
      console.log(
        `[${serverId}] Tool summary - Available: ${summary.available}, Unavailable: ${summary.unavailable}`,
      );
    }
  }
}
