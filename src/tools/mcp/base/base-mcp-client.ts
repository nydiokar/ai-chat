...
export class BaseMCPClient implements IMCPClient {
  ...
  public async listTools(): Promise<ToolDefinition[]> {
    try {
      if (!this.isConnected) {
        console.warn(
          `[${this.serverId}] Client not connected, attempting to connect...`,
        );
        await this.connect();
      }
      ...
      const response = await this.client.request(
        ...
        ListToolsResultSchema,
      ); // Using SDK's schema for validation
  ...
  public async listResources(query?: ResourceQuery): Promise<Resource[]> {
    ...
    console.warn(
      `[${this.serverId}] Resource methods are not fully implemented yet`,
    );
    return [];
  }
  ...
  public async createResource(params: ResourceCreateParams): Promise<Resource> {
    ...
    throw MCPError.toolExecutionFailed(
      new Error("Resource creation not supported"),
    );
  }
  ...
  public async updateResource(
    ...
    throw MCPError.toolExecutionFailed(
      new Error("Resource update not supported"),
    );
  }
  ...
  public async deleteResource(id: string): Promise<void> {
    ...
    throw MCPError.toolExecutionFailed(
      new Error(`Resource deletion not supported for ${id}`),
    );
  }
}
