...
export class MCPContainer {
  ...
  private registerClients(): void {
    ...
    for (const [serverId, serverConfig] of Object.entries(
      ...
      console.log(
        `MCPContainer: Created ${this.hasEnhancedFeatures() ? "enhanced" : "base"} client for server ${serverId}`,
      );
    ...
    this.container
      .bind<Map<string, string>>("ClientsMap")
      .toConstantValue(clientsMap);
    this.container
      .bind<Map<string, ServerConfig>>("ServerConfigs")
  ...
  private registerToolManagers(): void {
    if (this.hasEnhancedFeatures()) {
      this.container
        .bind<IToolManager>(TYPES.IToolManager)
        .to(EnhancedToolsHandler)
        .inSingletonScope();
    } else {
      this.container
        .bind<IToolManager>(TYPES.IToolManager)
  ...
  private registerServerManagers(): void {
    if (this.hasEnhancedFeatures()) {
      this.container
        .bind<IServerManager>(TYPES.IServerManager)
        .to(EnhancedServerManager)
        .inSingletonScope();
    } else {
      this.container
        .bind<IServerManager>(TYPES.IServerManager)
        .to(BaseServerManager)
        .inSingletonScope();
    }
  }
  ...
  private hasEnhancedFeatures(): boolean {
    return Object.values(this.config.features.enhanced).some(
      (enabled) => enabled,
    );
  }
  ...
  public getMCPClient(serverId: string): IMCPClient {
    ...
    if (!clientId) {
      throw new Error(
        `No client configuration found for server ${serverId}. Available servers: ${Array.from(clientsMap.keys()).join(", ")}`,
      );
    }
