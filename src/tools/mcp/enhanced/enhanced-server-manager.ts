...
export class EnhancedServerManager extends BaseServerManager {
  ...
  constructor(
    ...
    info(
      "Initializing Enhanced Server Manager",
      createLogContext(COMPONENT, "constructor", {
  ...
  private startHealthCheck(): void {
    if (this.healthCheckTimer) {
      debug(
        ...
        createLogContext(COMPONENT, "startHealthCheck", { status: "skipped" }),
      );
      return;
    }
    ...
    info(
      ...
      createLogContext(COMPONENT, "startHealthCheck", {
  ...
  private setupEnhancedClientListeners(): void {
    ...
    debug(
      ...
      createLogContext(COMPONENT, "setupEnhancedClientListeners", {
        status: "ready",
      }),
    );
  }
    ...
    try {
      ...
      for (const [serverId, clientId] of this.clientsMap.entries()) {
        if (!this.container.isBound(clientId)) {
          debug(
            "Skipping unbound client",
            createLogContext(COMPONENT, "updateManagedClients", {
              serverId,
              clientId,
              status: "skipped",
            }),
      ...
      error(
        "Client management update failed",
        createErrorContext(
          COMPONENT,
          "updateManagedClients",
          "System",
  ...
  private setupClientEventForwarding(
    serverId: string,
    ...
    debug(
      "Client event forwarding configured",
      createLogContext(COMPONENT, "setupClientEventForwarding", {
        serverId,
        status: "ready",
      }),
    );
  ...
  private async checkServerHealth(): Promise<void> {
    const activeServers = Array.from(this.servers.entries()).filter(
      ([_, server]) => server.state === ServerState.RUNNING,
    );

    debug(
      "Starting health check",
      createLogContext(COMPONENT, "checkServerHealth", {
    ...
    for (const [id, server] of activeServers) {
      ...
      try {
        ...
        if (!validationResult.success) {
          warn(
            "Server state validation failed",
            createLogContext(COMPONENT, "checkServerHealth", {
              serverId: id,
  ...
  private initializeHistory(id: string): void {
    if (this.serverHistory.has(id)) {
      debug(
        "Server history already exists",
        createLogContext(COMPONENT, "initializeHistory", {
          serverId: id,
          status: "exists",
        }),
      );
  ...
  private trackEvent(id: string, event: ServerEvent): void {
    ...
    if (history.events.length > this.MAX_HISTORY_SIZE) {
      ...
      debug(
        "History trimmed",
        createLogContext(COMPONENT, "trackEvent", {
          serverId: id,
      ...
      case "response":
        if (event.duration !== undefined) {
          history.metrics.avgResponseTime =
            (history.metrics.avgResponseTime * (history.events.length - 1) +
              event.duration) /
  ...
  public override async startServer(id: string): Promise<Server> {
    info(
      "Starting server",
      createLogContext(COMPONENT, "startServer", { serverId: id }),
    );
    ...
    try {
      ...
      info(
        ...
        createLogContext(COMPONENT, "startServer", {
          serverId: id,
          state: server.state,
          status: "success",
        }),
  ...
  public async pauseServer(id: string): Promise<void> {
    info(
      "Pausing server",
      createLogContext(COMPONENT, "pauseServer", { serverId: id }),
    );
    ...
    try {
      ...
      if (server.state !== ServerState.RUNNING) {
        warn(
          "Cannot pause server - invalid state",
          createLogContext(COMPONENT, "pauseServer", {
            serverId: id,
  ...
  public async resumeServer(id: string): Promise<void> {
    info(
      "Resuming server",
      createLogContext(COMPONENT, "resumeServer", { serverId: id }),
    );
    ...
    try {
      ...
      if (server.state !== ServerState.PAUSED) {
        warn(
          "Cannot resume server - invalid state",
          createLogContext(COMPONENT, "resumeServer", {
            serverId: id,
            currentState: server.state,
  ...
  public override async unregisterServer(id: string): Promise<void> {
    info(
      ...
      createLogContext(COMPONENT, "unregisterServer", { serverId: id }),
    );
    ...
    try {
      ...
      info(
        ...
        createLogContext(COMPONENT, "unregisterServer", {
          serverId: id,
  ...
  public cleanup(): void {
    info("Starting cleanup", createLogContext(COMPONENT, "cleanup"));
    ...
    try {
      if (this.healthCheckTimer) {
        ...
        debug(
          ...
          createLogContext(COMPONENT, "cleanup", { healthCheckStopped: true }),
        );
      }
      ...
      info(
        "Cleanup completed",
        createLogContext(COMPONENT, "cleanup", {
          status: "success",
          clearedServers: true,
          clearedClients: true,
        }),
   ...
   */
  protected override getClientFactory(): (
    config: ServerConfig,
    id: string,
  ) => IMCPClient {
    return (config: ServerConfig, id: string): IMCPClient => {
      ...
      if (existingClient) {
        debug(
          ...
          createLogContext(COMPONENT, "getClientFactory", { serverId: id }),
        );
