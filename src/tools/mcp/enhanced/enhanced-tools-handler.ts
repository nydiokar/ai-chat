...
export class EnhancedToolsHandler extends BaseToolManager {
  ...
  constructor(
    ...
    info(
      "Enhanced Tools Handler initialized",
    ...
    try {
      ...
      for (const serverId of serverIds) {
        ...
        client.on("tools.changed", async () => {
          debug(
            "Tool change event",
            createLogContext(COMPONENT, "setupClientEventListeners", {
              serverId,
              event: "tools.changed",
              action: "refresh",
            }),
      ...
      const enhancedClients = Array.from(this.clientsMap.values()).filter(
        (client) => client instanceof EnhancedMCPClient,
      ) as EnhancedMCPClient[];
  ...
  public override async getAvailableTools(): Promise<ToolDefinition[]> {
    ...
    info(
      "Tools retrieved",
      createLogContext(COMPONENT, "getAvailableTools", {
  ...
  public override async executeTool(
    ...
    try {
      debug(
        ...
        createLogContext(COMPONENT, "executeTool", {
          toolName: name,
  ...
  public override async refreshToolInformation(): Promise<void> {
    try {
      debug(
        "Starting tool information refresh",
        createLogContext(COMPONENT, "refreshToolInformation", {
      ...
      info(
        "Tool information refresh completed",
        createLogContext(COMPONENT, "refreshToolInformation"),
      );
    } catch (err) {
      error(
        "Tool information refresh failed",
        createErrorContext(
          COMPONENT,
  ...
  private initializeContext(toolName: string): void {
    if (!this.toolContexts.has(toolName)) {
      ...
      debug(
        "Tool context initialized",
        createLogContext(COMPONENT, "initializeContext", { toolName }),
      );
    }
  }
    ...
    try {
      ...
      if (history.length >= this.MAX_HISTORY_SIZE) {
        ...
        debug(
          ...
          createLogContext(COMPONENT, "trackToolUsage", {
      ...
      throw MCPError.toolTrackingFailed(
  ...
  public getToolAnalytics(name: string): ToolAnalytics | undefined {
    ...
    if (context.successRate < 0.5) {
      recommendations.push(
        "Tool has a low success rate. Consider reviewing error patterns.",
      );
    }

    if (context.averageExecutionTime > 5000) {
      recommendations.push(
        "Tool has high average execution time. Consider optimization.",
      );
    }
  ...
  public async getToolByName(
    ...
    try {
      ...
      if (tool) {
        ...
        debug(
          "Tool retrieved",
          createLogContext(COMPONENT, "getToolByName", {
            toolName: name,
            status: "found",
          }),
        );
        ...
        warn(
          "Tool not found",
          createLogContext(COMPONENT, "getToolByName", { toolName: name }),
        );
      }
