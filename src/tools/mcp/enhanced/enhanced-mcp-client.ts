...
export class EnhancedMCPClient extends BaseMCPClient {
  ...
  constructor(
    ...
    info(
      "Enhanced MCP Client initialized",
  ...
  private setupHealthMonitoring(): void {
    ...
    debug(
      ...
      createLogContext(COMPONENT, "setupHealthMonitoring", {
        serverId: this.serverId,
  ...
  private setupNotificationHandlers(): void {
    try {
      ...
      if (!this.client) {
        warn(
          "No client available for polling setup",
          createLogContext(COMPONENT, "setupNotificationHandlers", {
            serverId: this.serverId,
          }),
        );
        return;
      ...
      setInterval(async () => {
        try {
          ...
          error(
            "Error polling for tool changes",
            createErrorContext(
              COMPONENT,
    ...
    try {
      ...
      if (cachedTools) {
        ...
        if (cachedTools.length !== latestTools.length) {
          info(
            "Tool count changed",
            createLogContext(COMPONENT, "refreshTools", {
              serverId: this.serverId,
          ...
          if (hasNewTools) {
            ...
            this.eventEmitter.emit("tools.changed", {
              serverId: this.serverId,
            });
          }
        }
  ...
  private async checkHealth(): Promise<void> {
    ...
    try {
      ...
      info(
        "Health status",
        createLogContext(COMPONENT, "checkHealth", {
          serverId: this.serverId,
          status: HEALTH_STATUS.HEALTHY,
          timeSinceLastCheck,
          connectionRestored: wasReconnecting,
          previousAttempts: wasReconnecting
  ...
  private async handleReconnect(): Promise<void> {
    ...
    try {
      ...
      info(
        "Reconnection status",
        createLogContext(COMPONENT, "handleReconnect", {
          serverId: this.serverId,
          status: "success",
    ...
    try {
      ...
      this.metrics.successRate =
        this.metrics.requests === 0
          ? 1.0
          : (this.metrics.requests - this.metrics.errors) /
            this.metrics.requests;
  ...
  private updateResponseTimeMetrics(newTime: number): void {
    ...
    this.metrics.avgResponseTime =
      this.metrics.responseTimeData.reduce((sum, time) => sum + time, 0) /
  ...
  public getCachedValue<T>(key: string): T | null {
    ...
    if (!cached) {
      debug(
        "Cache operation",
        createLogContext(COMPONENT, "getCachedValue", {
          serverId: this.serverId,
          key,
          result: "miss",
        }),
  ...
  public async connect(): Promise<void> {
    try {
      info(
        ...
        createLogContext(COMPONENT, "connect", { serverId: this.serverId }),
      );
  ...
  private async checkNotificationSupport(): Promise<void> {
    try {
      ...
      const hasTools = await this.listTools()
        .then(() => true)
        .catch(() => false);
      const hasResources = await this.listResources()
        .then(() => true)
        .catch(() => false);

      debug(
        "Server capabilities checked",
        createLogContext(COMPONENT, "checkNotificationSupport", {
          serverId: this.serverId,
          hasTools,
          hasResources,
          status: "completed",
      ...
      error(
        "Failed to check notification support",
        createErrorContext(
          COMPONENT,
  ...
  public async disconnect(): Promise<void> {
    try {
      info(
        ...
        createLogContext(COMPONENT, "disconnect", { serverId: this.serverId }),
      );
  ...
  public resetMetrics(): void {
    info(
      "Resetting metrics",
      createLogContext(COMPONENT, "resetMetrics", {
        serverId: this.serverId,
