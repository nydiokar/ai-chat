 ...
 */
async function ensureServersRegistered(
  serverManager: BaseServerManager,
): Promise<void> {
  try {
    ...
    console.log(
      `Found ${configuredServers.length} servers in configuration: ${configuredServers.join(", ")}`,
    );
    ...
    console.log(
      `Registered ${successfulRegistrations.length}/${configuredServers.length} servers`,
    );
  } catch (error) {
...
async function main(): Promise<void> {
  try {
    ...
    console.log(
      `Dashboard server started. Access at http://localhost:${dashboardPort}`,
    );
