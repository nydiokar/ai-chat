...
export class MCPDashboard {
  ...
  private setupPeriodicRefresh(): void {
    ...
    this.periodicRefreshTimer = setInterval(() => {
      ...
      if (metrics.size === 0 || this._refreshCount % 5 === 0) {
        console.log(
          `[MCPDashboard] Periodic refresh #${this._refreshCount} - Current metrics count: ${metrics.size}`,
        );
      }
      ...
      if (metrics.size === 0) {
        console.log(
          "[MCPDashboard] No metrics found during periodic refresh, attempting registration",
        );
  ...
  private setupServerRegistrationCheck(): void {
    ...
    this.registrationTimer = setTimeout(() => {
      ...
      if (metrics.size === 0) {
        console.log(
          "[MCPDashboard] No servers discovered automatically. Attempting manual registration...",
        );
        this.directlyRegisterServers();
      }
  ...
  private directlyRegisterServers(): void {
    ...
    console.log(
      `[MCPDashboard] Manual registration check - ServerIds: ${serverIds.length}, Metrics: ${metrics.size}`,
    );
  ...
  public async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.port, () => {
        console.log(
          `[MCPDashboard] Dashboard is running at http://localhost:${this.port}/`,
        );
        resolve();
      });
    });
