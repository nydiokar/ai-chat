...
export class MetricsDashboard extends EventEmitter {
  ...
  private setupEventListeners(): void {
    ...
    this.serverManager.on(
      "server-state-change",
      (serverId: string, state: ServerState) => {
   ...
   */
  private setupClientMetricsListeners(
    serverId: string,
    ...
    try {
      ...
      if (!silent) {
        console.log(
          `Set up metrics listeners for client ${serverId} (metadata tracking only)`,
        );
      }
    } catch (error) {
      console.error(
        `Error setting up client metrics listeners for ${serverId}:`,
        error,
      );
    }
  }
  ...
  private getServersDirectly(): Map<string, Server> {
    try {
      ...
      if (manager.servers && manager.servers instanceof Map) {
        console.log(
          `[MetricsDashboard] Found servers map with ${manager.servers.size} servers: ${Array.from(manager.servers.keys()).join(", ")}`,
        );
      ...
      for (const propName of Object.getOwnPropertyNames(this.serverManager)) {
        ...
        if (prop instanceof Map && propName.toLowerCase().includes("server")) {
          ...
          console.log(
            `[MetricsDashboard] Accessing server map via ${propName}, found ${serverMap.size} servers: ${Array.from(serverMap.keys()).join(", ")}`,
          );
  ...
  private discoverServers(): void {
    ...
    if (directServerIds.length === 0 && this._lastDiscoveredCount === 0) {
      ...
      const managerMethods = Object.getOwnPropertyNames(
        Object.getPrototypeOf(this.serverManager),
      ).filter(
        (prop) => typeof (this.serverManager as any)[prop] === "function",
      );

      if (logMethods) {
        console.log(
          `[MetricsDashboard] No servers found via standard methods. Available methods: ${managerMethods.join(", ")}`,
        );
      }
    }
    ...
    if (
      directServerIds.length > 0 &&
      directServerIds.length !== this._lastDiscoveredCount
    ) {
      console.log(
        `[MetricsDashboard] Found ${directServerIds.length} servers via direct access`,
      );
    }
    ...
    if (directServerIds.length > 0) {
      ...
      if (
        serverIds.length > 0 &&
        serverIds.length !== this._lastDiscoveredCount
      ) {
        console.log(
          `[MetricsDashboard] Found ${serverIds.length} servers via interface methods`,
        );
      }
    ...
    if (discoveredServers.size !== this._lastDiscoveredCount) {
      if (discoveredServers.size > 0) {
        console.log(
          `[MetricsDashboard] Discovered ${discoveredServers.size} servers: ${Array.from(discoveredServers).join(", ")}`,
        );
      } else {
        console.log(
          `[MetricsDashboard] No servers discovered. This might indicate an issue with the server manager integration.`,
        );
      }
    ...
    if (server?.client) {
      try {
        ...
        if (error instanceof Error && error.message !== "Not found") {
          console.warn(
            `[MetricsDashboard] Error getting client metrics: ${error.message}`,
          );
        }
      }
    }
  ...
  private refreshAllMetrics(): void {
    ...
    if (serverIds.length > 0) {
      console.log(
        `[MetricsDashboard] Refreshing metrics for ${serverIds.length} servers`,
      );
    }
    ...
    if (this.serverMetrics.size !== this._lastMetricsCount) {
      console.log(
        `[MetricsDashboard] Metrics updated, current count: ${this.serverMetrics.size}`,
      );
  ...
  private refreshServerMetrics(serverId: string): void {
    ...
    if (!server) {
      ...
      if (this.serverMetrics.has(serverId)) {
        console.log(
          `[MetricsDashboard] Server ${serverId} not found, removing metrics`,
        );
  ...
  private updateClientMetrics(
    ...
    if (clientMetrics.requests > 0) {
      serverMetrics.successRate =
        (clientMetrics.requests - clientMetrics.errors) /
  ...
  public generateHtmlReport(): string {
    console.log(
      `[MetricsDashboard] Generating HTML report for ${this.serverMetrics.size} servers`,
    );
    ...
    const allServerIds = [
      ...new Set([...managerServerIds, ...metricServerIds]),
    ];

    console.log(
      `[MetricsDashboard] Server IDs from manager: ${managerServerIds.length}, from metrics: ${metricServerIds.length}, combined: ${allServerIds.length}`,
    );
    ...
    if (allServerIds.length === 0) {
      ...
      for (const serverId of allServerIds) {
        ...
        if (server) {
          ...
          else if (metrics.connectionState === "connecting")
            stateClass = "connecting";
        }
        ...
        if (metrics) {
          if (!server) {
            // Don't duplicate info if we already showed server data
            html += `
                            <tr>
                                <td>Uptime</td>
  ...
  public registerServer(serverId: string, server?: Server): void {
    ...
    try {
      ...
      if (serverObj) {
        console.log(
          `[MetricsDashboard] Found server ${serverId} via getServer method`,
        );
        ...
        console.log(
          `[MetricsDashboard] Server ${serverId} not found via getServer method`,
        );
      }
      ...
      if (mapServer) {
        console.log(
          `[MetricsDashboard] Found server ${serverId} via direct map access`,
        );
        ...
        console.log(
          `[MetricsDashboard] Server ${serverId} not found in direct map access`,
        );
      }
      ...
      if (serverIds.includes(serverId)) {
        console.log(
          `[MetricsDashboard] Server ${serverId} exists in getServerIds but can't be accessed via getServer`,
        );
      }
      ...
      console.log(
        `[MetricsDashboard] Server manager properties: ${managerProps.join(", ")}`,
      );
      ...
      console.log(
        `[MetricsDashboard] Created minimal metrics for manually registered server: ${serverId}`,
      );
    } catch (error) {
      console.error(
        `[MetricsDashboard] Error manually registering server ${serverId}:`,
        error,
      );
