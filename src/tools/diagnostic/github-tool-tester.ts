...
export async function runGitHubDiagnostic() {
  logger.info(
    "Starting GitHub Tool diagnostic",
    createLogContext("GitHubDiagnostic", "runGitHubDiagnostic", {
      status: "starting",
    }),
  );
  ...
  try {
    ...
    // Test complete
    logger.info(
      "GitHub Tool diagnostic complete",
      createLogContext("GitHubDiagnostic", "runGitHubDiagnostic", {
...
if (isMainModule) {
  runGitHubDiagnostic().catch((err) => {
    error(
      "Unhandled exception in GitHub diagnostic",
      createErrorContext(
        "GitHubDiagnostic",
        "main",
        "System",
