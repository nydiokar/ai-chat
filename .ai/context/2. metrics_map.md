...
*   **Primary Output Type**: `BehavioralMetrics` object, which includes:
    *   **Metrics**: `buySellRatio`, `buySellSymmetry`, `averageFlipDurationHours`, `medianHoldTime`, `sequenceConsistency`, `flipperScore` ## DEPRECATED (overall ratio penalty removed from calculation).
...
| Metric                          | Source Analyzer/Service      | Output Structure/Type      | Notes                                                                    |
|---------------------------------|------------------------------|----------------------------|--------------------------------------------------------------------------|
| **Metrics**             |                              |                            | (From `BehaviorAnalyzer`, in `BehavioralMetrics`)                        |
...
| `flipperScore` ## DEPRECATED ##                  | `BehaviorAnalyzer`           | `BehavioralMetrics`        | Score (0-1) indicating flipper-like behavior. (Overall ratio penalty removed). |
...
| `tradingStyle`                  | `BehaviorAnalyzer`           | `BehavioralMetrics`        | e.g., "", "Accumulator".                                     |
...
| `flipperScore`                     | `BehaviorAnalyzer`      | `BehavioralMetrics`   | **DEPRECATED** (Use `velocityScore` instead). Score (0-1) indicating flipper-like behavior. Replaced by velocityScore for clarity. |
| `velocityScore`                    | `BehaviorAnalyzer`      | `BehavioralMetrics`   | **Implemented** (New; replaces flipperScore). Score (0-100) measuring trading speed intensity. Uses logarithmic scale: `100 * (1 - log10(medianHoldTimeHours + 1) / log10(721))`. <1min=100, 1hr≈90, 24hr≈60, 7d≈25, 30d=0. Professional terminology for velocity-based classification. |
| **Activity Metrics**               |                         |                       | (From `BehaviorAnalyzer`, in `BehavioralMetrics`)                                                                                                       |
...
| **Classification**                 |                         |                       | (From `BehaviorAnalyzer`, in `BehavioralMetrics`)                                                                                                       |
| `tradingStyle`                     | `BehaviorAnalyzer`      | `BehavioralMetrics`   | Implemented. Classified trading style, e.g., 'Accumulator', based on behavioral metrics.                                              |
